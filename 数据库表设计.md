...
create table `user` (
  ...
  `password` char(64) NOT NULL comment '登录密码',
	`state` enum('verify_user', 'refuse_user', 'normal_user', 'verify_admin', 'normal_admin', 'root') DEFAULT 'verify_user' comment '审核用户、用户、审核管理员、管理员、用户审核不通过',
...
create table `appointment` (
  ...
  `emailed` tinyint(1) DEFAULT 0 comment '是否已经发送邮件',
  PRIMARY KEY(`id`),
