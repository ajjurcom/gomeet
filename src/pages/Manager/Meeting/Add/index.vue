<template>
    <div class="container-wrap">
        <custom-menu activeItem="meeting-add"></custom-menu>
        <div class="container">
            <Form class="meeting-form" ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
                <FormItem label="会议室名字" prop="meeting_name">
                    <Input v-model="formValidate.meeting_name" placeholder="会议室名字"></Input>
                </FormItem>
    ...
    methods: {
        handleSubmit (name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    this.loading = true;
                    this.$service.MainAPI.addMeeting(this.formValidate).then(() => {
